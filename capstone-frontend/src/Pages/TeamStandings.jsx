...
const TeamStandingsV2 = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const easternDefensiveTeam = easternTeams.reduce(
          (prev, current) => (prev.points.against < current.points.against ? prev : current),
          easternTeams[0]
        ).team
        const westernDefensiveTeam = westernTeams.reduce(
          (prev, current) => (prev.points.against < current.points.against ? prev : current),
          westernTeams[0]
        ).team
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      ...
      try {
        ...
        const easternTeams = response.data.standings.entries.filter((e) =>
          WNBAEasternConference.includes(e.team.displayName)
        )
        const westernTeams = response.data.standings.entries.filter((e) =>
          WNBAWesternConference.includes(e.team.displayName)
        )
        ...
        const easternDefensiveTeam = easternTeams.reduce(
          (prev, current) => (prev.stats[0].value < current.stats[0].value ? prev : current),
          easternTeams[0]
        ).team
        const westernDefensiveTeam = westernTeams.reduce(
          (prev, current) => (prev.stats[0].value < current.stats[0].value ? prev : current),
  ...
  const WNBATeamData = WNBADATA.map((team) => {
    return {
      name: team.name,
      logo: team.avatarSrc,
    }
  ...
  return (
    ...
    >
      <Block
        ...
        height="60px"
        marginBottom={
          data == null || data === undefined || data.length === 0 || !data ? '35px' : '0px'
        }
      >
        <Block
        ...
        >
          <Link
            href={league === 'NBA' ? 'https://www.nba.com/' : 'https://www.wnba.com/'}
            target="_blank"
          ...
          >
            {league === 'NBA' ? (
              ...
              <img
                ...
                alt="WNBA Logo"
                style={{
                  height: screenWidth > 530 ? '20px' : '25px',
          ...
          {screenWidth > 530 ? (
            <HeadingLevel>
              {league === 'NBA' ? (
                ...
                <Heading styleLevel={screenWidth > 815 ? 5 : 6} color="black">
                  {' '}
                  WNBA 2025 Season{' '}
                </Heading>
              )}
            ...
            <HeadingXSmall marginLeft="5px" color="black">
              WNBA 2025
            </HeadingXSmall>
          )}
        </Block>
        ...
        <Block
          ...
          $style={{
            marginLeft: `-${130}px`,
            marginRight:
              screenWidth <= 530 ? `${15}px` : screenWidth < 1400 ? `${50}px` : `${-5}px`,
          }}
        >
          <Block marginRight="10px" paddingTop="10px">
            <Select
              ...
              overrides={{
                ...
                SingleValue: {
                  style: { lineHeight: '30px', fontSize: screenWidth > 530 ? 'inherit' : '14px' },
                },
          ...
          >
            <Block>
              <TopScoringTeamCard
                logo={
                  league === 'NBA'
                    ? westernTopScoringTeam?.logo
                    : WNBAWesternTopScoringTeam?.logos?.[0]?.href
                }
                name={
                  league === 'NBA'
                    ? westernTopScoringTeam?.name
                    : WNBAWesternTopScoringTeam?.displayName
                }
                conference="Western"
                league={league}
              />
            ...
            <Block>
              <TopDefensiveTeamCard
                logo={
                  league === 'NBA'
                    ? westernTopDefensiveTeam?.logo
                    : WNBAWesternTopDefensiveTeam?.logos?.[0]?.href
                }
                name={
                  league === 'NBA'
                    ? westernTopDefensiveTeam?.name
                    : WNBAWesternTopDefensiveTeam?.displayName
                }
                conference="Western"
                league={league}
              />
        ...
        {league === 'NBA' ? (
            ...
            {data == null || data === undefined || data.length === 0 || !data ? (
              ...
              <Block
                ...
                $style={{
                  ...
                  ...(screenWidth > 1305 ? { maxWidth: '850px' } : {}),
                }}
              >
                <HeadingMedium marginLeft="5px" color="white">
                  Western Conference
                </HeadingMedium>
              </Block>
            )}
            {data == null || data === undefined || data.length === 0 || !data ? (
              ...
              >
                <TableBuilderColumn header="Team">
                  {(row) => (
                    <AvatarCell src={row.avatarSrc} title={row.name} subtitle={row.title} />
                  )}
                </TableBuilderColumn>
          ...
          <Block
            ...
            style={{
              ...
              marginBottom: window.innerWidth > 1305 ? 'inherit' : '200px',
            }}
          >
            <Block
              display="flex"
              ...
              backgroundColor="black"
              width="100%"
              marginTop={window.innerWidth > 1305 ? '-190px' : '5px'}
              $style={{
                borderTopLeftRadius: '8px',
                borderTopRightRadius: '8px',
            ...
            >
              {screenWidth > 815 ? (
                ...
                <HeadingXSmall marginLeft="5px" color="white">
                  Overall Standings
                </HeadingXSmall>
              ) : (
                <HeadingSmall marginLeft="5px" color="white">
                  Overall Standings
                </HeadingSmall>
              )}
            </Block>
          ...
          >
            <Block>
              <TopScoringTeamCard
                logo={
                  league === 'NBA'
                    ? easternTopScoringTeam?.logo
                    : WNBAEasternTopScoringTeam?.logos?.[0]?.href
                }
                name={
                  league === 'NBA'
                    ? easternTopScoringTeam?.name
                    : WNBAEasternTopScoringTeam?.displayName
                }
                conference="Eastern"
              />
            </Block>
            <Block>
              ...
              <TopDefensiveTeamCard
                logo={
                  league === 'NBA'
                    ? easternTopDefensiveTeam?.logo
                    : WNBAEasternTopDefensiveTeam?.logos?.[0]?.href
                }
                name={
                  league === 'NBA'
                    ? easternTopDefensiveTeam?.name
                    : WNBAEasternTopDefensiveTeam?.displayName
                }
                conference="Eastern"
              />
            </Block>
      ...
      >
        <HeadingLevel>
          <Heading
            className="headingStandings"
            styleLevel={screenWidth > 1015 ? 4 : screenWidth > 815 ? 5 : 6}
            color="white"
            style={{
              display: 'flex',
              paddingTop: '5px',
          ...
          >
            {league === 'NBA' ? `NBA h2h Ventures` : `WNBA h2h Ventures`}
          </Heading>
        </HeadingLevel>
      </Block>
      <Block
      ...
      >
        ...
        >
          {(teamData?.eastern?.length > 0 && league === 'NBA') ||
          (WNBATeamData.length > 0 && league === 'WNBA') ? (
            <GameOdds
              teamData={league === 'NBA' ? teamData : WNBATeamData}

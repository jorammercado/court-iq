...
const NBATeams = ({}) => {
  ...
  useEffect(() => {
    ...
    else setAvatarBackgroundColor('none')
  }, [selectedTeamName])
  ...
  function selectFontFamily(selectedTeamName) {
    for (let i = 0; i < teams.length; i++) {
      if (
        teams[i] === selectedTeamName &&
        fontsfamilies.includes(teams[i].split(' ')[teams[i].split(' ').length - 1].toLowerCase())
      )
        return fontsfamilies[
          fontsfamilies.indexOf(teams[i].split(' ')[teams[i].split(' ').length - 1].toLowerCase())
        ]
    }
  ...
  function selectPrimaryColor(selectedTeamName) {
    for (let i = 0; i < teams.length; i++) {
      if (teams[i] === selectedTeamName) return primaryColors[i]
    }
    return '#EA6607'
  }
  ...
  function selectSecondaryColor(selectedTeamName) {
    for (let i = 0; i < teams.length; i++) {
      if (teams[i] === selectedTeamName) return secondaryColors[i]
    }
    return '#000000'
  }
  ...
  return (
    <Block className="parent" style={{ position: 'relative', zIndex: 0 }}>
      <Block className="left">
        <Block
          className="team__logoNBA"
          $style={{ flexGrow: 1, marginLeft: `${marginLeft + 15}px` }}
        >
          <Avatar
            overrides={{
              Avatar: {
      ...
      <Block className="backgroundWrapper" backgroundColor={primaryColor}>
        <Block className="middle">
          <Block
            className="team"
            $style={{ marginBottom: screenWidth >= 820 ? 'inherit' : '25px' }}
          >
            <HeadingLevel>
              <Heading
                styleLevel={screenWidth >= 820 ? 1 : 4}
                marginTop="10px"
              ...
              <Heading
                marginTop="-5px"
                styleLevel={screenWidth >= 820 ? 3 : 6}
                color={secondaryColor}
                style={{ fontFamily: fontFamily }}
              >
                {season ? season + `-${(Number(season) + 1).toString()}` : ''}
              </Heading>
            </HeadingLevel>
          </Block>
          ...
          <Block className="teamsItems">
            <TeamStats teamId={teamId} season={season} isHighlightedSeason={isHighlightedSeason} />
            <TeamSchedule
              teamId={teamId}
              season={season}
      ...
      <Block className="right">
        <Block
          ...
          marginBottom="70px"
          $style={{
            marginRight: screenWidth < 500 ? `${marginLeft * 0.03}px` : `${marginLeft - 12}px`,
          }}
        >
          <Block marginRight="10px" paddingTop="10px">
            <Select
              ...
              overrides={{
                ...
                SingleValue: {
                  style: { lineHeight: '30px', fontSize: screenWidth > 400 ? 'inherit' : '14px' },
                },

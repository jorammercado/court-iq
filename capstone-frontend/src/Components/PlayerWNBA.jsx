...
import { LabelLarge, LabelMedium, HeadingMedium, HeadingSmall } from 'baseui/typography'
...
function PlayerWNBA({ data, playerid }) {
  ...
  function selectPrimaryColor(referenceData) {
    if (referenceData && referenceData.displayName)
      for (let i = 0; i < teams.length; i++) {
        if (teams[i] === referenceData.displayName) return primaryColors[i]
      }
    return '#EA6607'
  }
  ...
  function selectSecondaryColor(referenceData) {
    if (referenceData && referenceData.displayName)
      for (let i = 0; i < teams.length; i++) {
        if (teams[i] === referenceData.displayName) return secondaryColors[i]
      }
    return '#000000'
  }
        ...
        >
          ...
          >
            <Block className="head__shot" $style={{ marginBottom: '-6px' }}>
              <img
                src={
                  playerImage || 'https://cdn.nba.com/headshots/nba/latest/1040x760/fallback.png'
                }
                alt="Head Shot"
                style={{ height: '240px' }}
              />
            ...
            >
              <HeadingLevel>
                <Heading styleLevel={2} color={secondaryColor}>

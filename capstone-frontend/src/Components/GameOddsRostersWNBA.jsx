...
const GameOddsRosters = ({ eventId, teamName }) => {
  ...
  useEffect(() => {
    ...
    const fetchPlayerProps = async (eventId) => {
      setIsLoading(true);
      try {
        const response = await axios.get(`https://api.the-odds-api.com/v4/sports/basketball_wnba/events/${eventId}/odds`, {
          params: { apiKey, regions: 'us', markets: 'player_points,player_rebounds,player_assists,player_threes,player_blocks', 
          oddsFormat: 'american' },
        });
        ...
        if (response && response.data && response.data.bookmakers && response.data.bookmakers[0] &&
          ...
          const allBookmakers = response.data.bookmakers.map(bookmaker => {
            return {
              ...
              lastUpdate: bookmaker.markets && bookmaker.markets.length > 0 ? bookmaker.markets[1].last_update : 'No update',
              playerProps: bookmaker.markets && bookmaker.markets.length > 0 ? bookmaker.markets[1].outcomes : []
            };
          });

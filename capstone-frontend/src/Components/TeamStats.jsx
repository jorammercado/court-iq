...
const TeamStatsComponent = ({ teamId, season, isHighlightedSeason }) => {
      ...
      try {
        ...
        if (response.data.response && response.data.response.length > 0) {
          ...
          let averages = Object.keys(totals).reduce((acc, stat) => {
            acc[stat] =
              stat === 'games' ? totals[stat] : parseFloat((totals[stat] / totals.games).toFixed(2))
            return acc
          }, {})
  ...
  const statColumns = Object.keys(teamStats[0])
    ...
    .map((key) => ({
      header:
        key.charAt(0).toUpperCase() + key.slice(1).replace(/[A-Z]/g, (letter) => ` ${letter}`), // Improve readability of acronyms
      id: key,
    }))

  return (
    <Block
      className="TeamStatsTable1"
      style={{
        justifyContent: 'left',
    ...
    >
      ...
      >
        <Block
          className="heading"
          backgroundColor={isHighlightedSeason ? '#EA6607' : 'transparent'}
          width="100%"
        >
          <HeadingLevel>
            <Heading

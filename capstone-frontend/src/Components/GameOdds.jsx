    ...
    if (teamData) {
      const fetchOdds = async () => {
        ...
        try {
          ...
          const enrichedOdds = response.data
            .map((game) => {
              ...
              if (market) {
                return {
                  ...
                  homeLogo:
                    sportInput === 'basketball_nba'
                      ? teamData.eastern.find((t) => t.name === game.home_team)?.logo ||
                        teamData.western.find((t) => t.name === game.home_team)?.logo
                      : teamData.find((t) => t.name === game.home_team)?.logo ||
                        teamData.find((t) => t.name === game.home_team)?.logo,
                  awayLogo:
                    sportInput === 'basketball_nba'
                      ? teamData.eastern.find((t) => t.name === game.away_team)?.logo ||
                        teamData.western.find((t) => t.name === game.away_team)?.logo
                      : teamData.find((t) => t.name === game.away_team)?.logo ||
                        teamData.find((t) => t.name === game.away_team)?.logo,
                }
              }
              return null
          ...
          const enrichedOdds2 = response.data
            .map((game) => {
          ...
          const enrichedOdds3 = response.data
            .map((game) => {
          ...
          const enrichedOdds4 = response.data
            .map((game) => {
  ...
  return (
    <div>
      <div className="ContentGameOdds">
        {draftKingsOdds?.length === 0 &&
        fanduelOdds?.length === 0 &&
        betMGMOdds?.length === 0 &&
        bovadaOdds?.length === 0 ? (
          <H2HCard
            key={0}
        ...
        {draftKingsOdds && draftKingsOdds.length > 0 ? (
          draftKingsOdds.map((game, index) => (
            <a
              href={urls['DraftKings']}
              target="_blank"
        ...
        {fanduelOdds && fanduelOdds.length > 0 ? (
          fanduelOdds.map((game, index) => (
            <a
              href={urls['FanDuel']}
              target="_blank"
              rel="noopener noreferrer"
        ...
        {betMGMOdds && betMGMOdds.length > 0 ? (
          betMGMOdds.map((game, index) => (
            <a
              href={urls['BetMGM']}
              target="_blank"
              rel="noopener noreferrer"
        ...
        {bovadaOdds && bovadaOdds.length > 0 ? (
          bovadaOdds.map((game, index) => (
            <a
              href={urls['Bovada']}
              target="_blank"
              rel="noopener noreferrer"

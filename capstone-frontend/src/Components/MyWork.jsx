...
const MyWork = () => {
  ...
  const variants = {
    ...
    offscreen: {
      x:
        screenWidth >= 1252 && screenWidth < 1350
          ? -divWidth
          : screenWidth >= 1252
            ? -2 * divWidth
            : -divWidth,
  ...
  return (
    ...
    >
      <div
        className="content"
        style={{ backgroundColor: '#faf7f2', boxShadow: 'none', borderStyle: 'none' }}
      >
        <header>
          <h2 style={{ ...(screenWidth <= 830 ? { fontSize: '23px' } : {}) }}>
            Comparison of Propositional and Head-to-Head Bets Across Different Bookmakers
          </h2>
          <h5
            className="deep-dive-message"
            style={{ ...(screenWidth <= 830 ? { fontSize: '13px' } : {}) }}
          >
            By exploring a multitude of{' '}
            <span style={{ color: '#EA6607' }}>
              different bookmakers odds for the same event or prop, we{' '}
            </span>
            <span style={{ color: '#EA6607' }}>enable users to</span> make more informed assumptions
            and <span style={{ color: '#EA6607' }}>obtain better outcomes</span>.
          </h5>
        </header>
